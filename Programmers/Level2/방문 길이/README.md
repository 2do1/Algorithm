...
테스트 케이스 8번부터 틀렸었는데 가는 경로와 되돌아오는 경로를 다른 경로라고 생각해서 그런것이었다.

예시로 "UDU"의 경우 1이 나와야 한다.

리스트에 "현재 위치 -> 이동 후 위치", "이동 후 위치 -> 현재 위치"를 넣어 지나간 경로인지의 여부를 in 함수를 이용해 판단하였다.

나는 리스트로 풀이했는데 다른 사람의 풀이를 보니 중복 제거를 하기 위해 set 집합 자료구조를 이용해 풀이했다. 단순 구현 문제였던 것 같다.