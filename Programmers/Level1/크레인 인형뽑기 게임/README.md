...
리스트.pop()은 리스트 맨 마지막 요소를 돌려주고 그 요소를 삭제해, 리스트 자체를 바꾸는 것이다.

조건식에 stack.pop()을 쓰니 인덱스 에러가 발생해 stack.pop() -> stack[-1]로 수정했다.

다른 사람의 코드를 보니 내 코드보다 더 깔끔하게 풀이 한것 같다. 내 코드가 너무 복잡한것 같기도 하다..

정답은 맞았지만 바구니가 비었을 경우와 같은 모양의 인형 두개가 연속해서 쌓이지 않을때의 경우의

코드가 똑같아 이 부분을 중복되지 않게 작성했으면 더 좋았을 것 같다.
