...
| :-------- | :---------- | :---- | :---- | :-------- | :-------- |
| 2 초      | 512 MB      | 31357 | 16969 | 12427     | 54.438%   |
...
- 처음에 회전시킨 톱니바퀴는 무조건 회전해야한다.
- 회전시키고 나서 옆의 톱니바퀴를 회전할 수 있는지 확인하는 것이 아니라, 회전하기 전에 옆의 톱니바퀴들이 모두 다 회전할 수있는지 확인해야 한다.
- 옆의 톱니바퀴들의 옆의 톱니바퀴들도 전부 확인해야한다.
- 회전 여부를 결정짓는 톱날은 인덱스 2번과 6번이다.

리스트를 회전하는 문제나 원형 돌리기 문제에서는 deque(), 덱 자료구조를 이용하면 된다.

deque.rotate() 함수를 이용하여 리스트를 회전할 수 있다.

음수를 넣게 되면 왼쪽회전, 양수를 넣게 되면 오른쪽회전한다.

이 문제에서도 덱 자료구조를 이용하였고, 톱니바퀴를 회전시킬 때 시계방향은 1, 반시계 방향은 -1을 파라미터로 rotate() 함수를 이용하면 됐다.
