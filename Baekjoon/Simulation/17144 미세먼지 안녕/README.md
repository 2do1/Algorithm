...
| :-------- | :---------- | :---- | :---- | :-------- | :-------- |
| 1 초      | 512 MB      | 30641 | 16838 | 11258     | 54.250%   |
...
PyPy3에서 통과했다. Python3에서는 시간초과가 발생

'미세먼지가 확산된다. 확산은 미세먼지가 있는 모든 칸에서 동시에 일어난다'. 라는 조건이 꽤 까다로웠다.

미세먼지가 확산할때 미세먼지가 있는 칸은 나중에 더해주기 위해 저장할 리스트를 생성하여 구현했다.

회전시켜야해서 덱 자료구조를 이용해야하나 싶었는데, 그냥 위쪽 아래쪽 나누어서 시계방향 반시계방향 회전을 구현하면 됐다. 이를 구현할 때 Swap을 이용해야 했다. 이 부분을 구현하는데 시간이 좀 걸렸다.

파이썬은 다음과 같이 손쉽게 Swap을 할 수 있다.
