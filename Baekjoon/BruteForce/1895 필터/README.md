...
![img](https://www.acmicpc.net/upload/images/filter1.gif)

필터링된 이미지의 크기는 4 × 3이고, 아래와 같다.

![img](https://www.acmicpc.net/upload/images/filter2.gif)

가장 왼쪽 윗 행에 필터를 두고, 오른쪽으로 움직이면서 중앙값을 찾는다. 한 행을 모두 이동했으면, 다음 행으로 이동해 다시 중앙값을 찾는다. 아래와 같은 순서를 가진다.

![img](https://www.acmicpc.net/upload/images/filter3.gif)
...
4중 for문으로 완전탐색하면 되는 문제였다.

for문의 범위만 잘 설정하면 쉽게 풀리는 문제였다.